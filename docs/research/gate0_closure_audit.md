...
detail, but it is **not execution-ready**. TB-0005 has converted previously
open-ended intervention, artifact, checkpoint, and staging questions into the
finite choices in the [decision dossier](gate0_decision_dossier.md). No
approval is implied. The remaining blockers form six auditable clusters:

1. **Scientific boundary and clinical construct:** scope narrowing, the exact
...
4. **Artifact claim and power:** the owner has not chosen diagnostic-veto versus
   powered bounded-probe evidence; exact probe pipelines are unspecified and
   present construct floors cannot establish four-probe equivalence.
5. **Checkpoint and breadth evidence:** no candidate pretrained VLM proves
...
6. **Resources and governance:** the successful simulation path reaches up to
   ...
   TB-0010's static design supplies a stage graph, proposed result/restart
   schema, proof register, and future generic-benchmark equations. TB-0011
   compiles a typed output/operation registry and corrects the conditional core
   floor to 613,093,770,610 bytes before unresolved typed extensions and
   overhead. Owner approval of the registry, CPU-core-hours, RAM, scratch,
   final persistent storage upper bound, wall time, cost, capacity, downstream
...
instrument identity reopens Gate 0 rather than selecting a winner from
protected outcomes.
Target prevalence/covariance and the complete downstream power calculation are
likewise a hard pre-confirmation lock, not evidence supplied by the construct
sample floor.
...
| **Decision-blocked** | A finite choice exists in the decision dossier, but the named owner has not selected it. |
| **Feasibility-blocked** | The answer requires a later bounded check in an approved environment. |
| **Specification-blocked** | The finite role is known but exact formula, implementation, revision, or legal identity is still missing. |
| **Not specified** | The contract requirement still lacks a defensible candidate. |
| **Method-claim killed** | A pre-data analytic screen rejected the named candidates; this is not an empirical null, equivalence, or subsumption result. |
...
| Compute and annotation budget | Explicit planning ceilings and stop rules are proposed; `G0-MV-Q A` reallocates 19 clinical hours, while TB-0009 enumerates 10,847 reliability and 2,438 pre-calibration `MV-1` cells. TB-0010 adds a static computation/schema/proof design; TB-0011 adds the typed registry/ledger and corrects the conditional core floor to 613,093,770,610 bytes before unresolved extensions. Neither establishes a final storage upper bound, CPU/RAM/scratch/wall-time allocation, or resource availability. | Candidate; simulation-, feasibility-, and approval-blocked | Choose `G0-RESOURCES A/B/C`. Before Gate-0 closure, Commander, clinical, statistical, resource, governance, and infrastructure owners must approve/amend the registry, close every extension/storage term, approve a workload-equivalent generic-kernel benchmark and conservative storage/contingency capacity, or approve a prospective proof-preserving redesign. Project RNG/DGP/calibration/bootstrap implementation remains post-Gate-0. Resource failure narrows or stops; it cannot lower fixed counts or gates. |
...
4. **Data/governance:** approve the pre-access MIMIC record, exact partitions,
   target sampling/weights/shift, secure processing, permitted derivatives,
...
7. **Pre-Month-3 lock:** after authorized development only, freeze the fitted
   instances/configurations of the already Gate-0-named estimator and matched
   deterministic comparator, plus every normalizer/code/probe/threshold choice,
   before bucket 70--84 is opened.
8. **Pre-confirmation lock:** verify a strict evidence tier or narrow the claim,
   and approve the patient-clustered construct/probe plus target-outcome power
   calculations before any confirmatory or natural-target set is opened.
