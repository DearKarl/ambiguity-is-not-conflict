...
1. **Scientific boundary and clinical construct:** the Commander selected
   `G0-SCOPE A`, but scientific-supervisor co-approval, the exact task/unit,
   ontology, `MV-1`/`MT-1`, reader design, and the later
...
6. **Resources and governance:** the successful simulation path reaches up to
   ...
   terms, and every accountable sign-off remain open. DR-0015 additionally
   rules out the Commander's local workstation for the approximately 613-GB
   conditional core-output route; that number is not the dataset size or a
   final storage upper bound.
...
1. **Scientific boundary:** the Commander has chosen the narrower determinate
   route; obtain scientific-supervisor co-approval before execution.
